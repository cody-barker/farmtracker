...
function FarmDetail({ allFarms, setAllFarms }) {
  ...
  if (farm) {
    tableRowComps = farm.beds.map((bed) => (
      <TableRow
        key={bed.id}
        bed={bed}
        allFarms={allFarms}
  ...
  return (
    <div className="farm-detail-container">
      <div className="farm-detail">
        ...
        <div className="button-container">
          <button onClick={onDeleteFarm} className="delete">
            Delete {farm.name}
          </button>
        </div>

        <div className="button-container">
          <button onClick={handleFormVis}>
            {showForm ? "Close Bed Form" : "Add a Bed"}
          </button>
        </div>

        {showForm ? (
          <AddABedForm allFarms={allFarms} setAllFarms={setAllFarms} />
        ) : null}
